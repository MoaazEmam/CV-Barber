...
parseBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 429) {
                const seconds = (data.detail || '').match(/\d+/)?.[0] || 60;
                showError(parseError, `Service is busy. Please wait ${seconds} seconds and try again.`);
            } else if (res.status === 503) {
                showError(parseError, 'Daily usage limit reached. The service resets at midnight. Please try again tomorrow.');
            } else {
...
tailorBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 429) {
                const seconds = (data.detail || '').match(/\d+/)?.[0] || 60;
                showError(tailorError, `Service is busy. Please wait ${seconds} seconds and try again.`);
            } else if (res.status === 503) {
                showError(tailorError, 'Daily usage limit reached. The service resets at midnight. Please try again tomorrow.');
            } else {
